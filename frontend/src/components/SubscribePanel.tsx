...
export function SubscribePanel({ scopes, onClose }: SubscribePanelProps) {
      ...
      <div className="rounded-2xl border border-ink/10 p-4 dark:border-line">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => void copy(key, url)} className="min-h-11 rounded-full border border-ink/15 px-4 text-sm font-semibold hover:bg-soft/25 dark:border-line-strong dark:hover:bg-white/10">
        ...
        />
        {outcome && (
          <p role="status" className={`mt-2 text-xs ${outcome.copied ? 'text-ink/60 dark:text-muted' : 'text-red-700 dark:text-red-300'}`}>
            {outcome.copied
  ...
  return (
    <Modal title="Choose your calendar" onClose={onClose} wide>
      <p className="mb-6 max-w-2xl text-sm leading-6 text-ink/65 dark:text-muted">
      ...
      <p className="mt-6 text-xs leading-5 text-ink/50 dark:text-muted">
