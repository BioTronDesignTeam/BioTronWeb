...
const SessionCookie = "exo_session"

// newToken returns a 256-bit URL-safe random token (used for session ids and
...
// hashToken returns the SHA-256 hex of a token. Sessions are stored by hash so
// the raw cookie value never touches the database.
