...
/**
 * Subscribe and the calendar filter. From lg up it sits beside the grid and
 ...
 */
export function Sidebar({ open, scopes, selectedScopes, onScopeChange, onSubscribe, onClose }: SidebarProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      ...
      >
        ...
        <p className="mt-auto px-2 text-xs text-ink/50 dark:text-faint">Times use America/Toronto. Calendar subscriptions update on each calendar app’s schedule.</p>
      </aside>
    </>
  );
}
